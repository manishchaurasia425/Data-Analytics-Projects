...
AS Average_Ordeer_Value FROM pizza_sales
...
SELECT pizza_category, SUM(total_price) AS Total_Saales , SUM(total_price) * 100 / ( SELECT(SUM(total_price)) 
FROM pizza_sales ) AS PCT FROM pizza_sales GROUP BY pizza_category
...
WHERE DATEPART(quarter , order_date)=1  GROUP BY pizza_size ORDER BY PCT DESC
